...
void __maybe_unused arduinotre_add_board_info(struct ti_common_eeprom *vep)
{
	vep->header = TI_EEPROM_HEADER_MAGIC;
	strncpy(vep->name, "TREBOARD", TI_EEPROM_HDR_NAME_LEN);
	strncpy(vep->version, "00A1", TI_EEPROM_HDR_REV_LEN);
	strncpy(vep->serial, "000000000000", TI_EEPROM_HDR_SERIAL_LEN);
}

int __maybe_unused arduinotre_virtual_eeprom_get(void)
{
	struct ti_common_eeprom *virtual_eeprom;

	virtual_eeprom = TI_EEPROM_DATA;
	if (virtual_eeprom->header != TI_EEPROM_HEADER_MAGIC)
		arduinotre_add_board_info(virtual_eeprom);
